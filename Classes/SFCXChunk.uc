class SFCXChunk extends Projectile;
...
simulated function HitWall( vector HitNormal, actor Wall )
{
		...
		&& ((Mover(Wall) == None) || Mover(Wall).bDamageTriggered) )
    {
        if ( Level.NetMode != NM_Client )
		{
            ...
            Wall.TakeDamage( Damage/2, instigator, Location, MomentumTransfer * Normal(Velocity), MyDamageType);
		}
        Destroy();
        return;
    }
	...
	if (Bounces > 0)
    {
        ...
        Velocity = 0.85 * (Velocity - 2.0*HitNormal*(Velocity dot HitNormal));
        Bounces = Bounces - 1;
        return;
    }
	bBounce = false;
